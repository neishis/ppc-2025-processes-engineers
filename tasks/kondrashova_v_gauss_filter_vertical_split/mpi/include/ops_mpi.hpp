...
class KondrashovaVGaussFilterVerticalSplitMPI : public BaseTask {
 ...
 private:
  ...
  void CalculateColumnDistribution(int width, int size, 
                                   std::vector<int>& col_counts, 
                                   std::vector<int>& col_offsets);
  
  void DistributeImageData(int rank, int size, int width, int height, int channels,
                           const std::vector<int>& col_counts,
                           const std::vector<int>& col_offsets,
                           std::vector<uint8_t>& local_data,
                           int extended_cols);
  
  void ApplyGaussFilterToLocalData(const std::vector<uint8_t>& local_data,
                                   std::vector<uint8_t>& local_result,
                                   int extended_cols, int local_cols,
                                   int height, int channels,
                                   int offset_in_extended);
  
  void GatherResults(int rank, int size, int width, int height, int channels,
                     const std::vector<int>& col_counts,
                     const std::vector<int>& col_offsets,
  ...
  uint8_t ApplyGaussToLocalPixel(const std::vector<uint8_t>& local_data, int local_width,
                                  int height, int channels, int x, int y, int channel) const;
};
