...
class KondrashovaVGaussFilterVerticalSplitMPI : public BaseTask {
 ...
 private:
  ...
  void CalculateColumnDistribution(int width, int size, std::vector<int> &col_counts, std::vector<int> &col_offsets);
  ...
  void ApplyGaussFilterToLocalData(const std::vector<uint8_t> &local_data, std::vector<uint8_t> &local_result,
                                   int extended_cols, int local_cols, int height, int channels, int offset_in_extended);
