...
const int KondrashovaVGaussFilterVerticalSplitMPI::kGaussKernel[3][3] = {
    {1, 2, 1},
    {2, 4, 2},
    {1, 2, 1}
};
...
void KondrashovaVGaussFilterVerticalSplitMPI::BroadcastImageDimensions(
...
void KondrashovaVGaussFilterVerticalSplitMPI::CalculateColumnDistribution(
    int width, int size, 
...
void KondrashovaVGaussFilterVerticalSplitMPI::DistributeImageData(
    int rank, int size, int width, int height, int channels,
    ...
    for (int p = 0; p < size; ++p) {
      ...
      if (p == 0) {
        ...
        MPI_Send(send_data.data(), static_cast<int>(send_data.size()), 
                 MPI_UINT8_T, p, 0, MPI_COMM_WORLD);
      }
    }
  } else {
    MPI_Status status;
    MPI_Recv(local_data.data(), static_cast<int>(local_data.size()), 
             MPI_UINT8_T, 0, 0, MPI_COMM_WORLD, &status);
  }
}

void KondrashovaVGaussFilterVerticalSplitMPI::ApplyGaussFilterToLocalData(
    const std::vector<uint8_t>& local_data,
    std::vector<uint8_t>& local_result,
    int extended_cols, int local_cols,
  ...
  for (int y = 0; y < height; ++y) {
    for (int lx = 0; lx < local_cols; ++lx) {
      ...
      for (int c = 0; c < channels; ++c) {
        ...
        local_result[result_idx] = ApplyGaussToLocalPixel(
            local_data, extended_cols, height, channels, x, y, c);
      }
    }
  }
}

void KondrashovaVGaussFilterVerticalSplitMPI::GatherResults(
    int rank, int size, int width, int height, int channels,
    const std::vector<int>& col_counts,
    const std::vector<int>& col_offsets,
    int local_start_col, int local_cols,
  ...
  if (rank == 0) {
    ...
    for (int p = 1; p < size; ++p) {
      ...
      MPI_Status status;
      MPI_Recv(recv_data.data(), static_cast<int>(recv_data.size()), 
               MPI_UINT8_T, p, 1, MPI_COMM_WORLD, &status);
    ...
    MPI_Send(local_result.data(), static_cast<int>(local_result.size()), 
             MPI_UINT8_T, 0, 1, MPI_COMM_WORLD);
  }
}

void KondrashovaVGaussFilterVerticalSplitMPI::BroadcastResultToAllProcesses(
    int width, int height, int channels) {
  int rank = 0;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  ...
  MPI_Bcast(GetOutput().pixels.data(), 
            static_cast<int>(GetOutput().pixels.size()), 
