...
## 2. Постановка задачи
**Цель работы:** Реализовать две версии алгоритма передачи данных по кольцевой топологии — последовательную (SEQ) с использованием MPI_Cart_create/MPI_Graph_create и параллельную (MPI) с явной маршрутизацией, провести их сравнение и анализ эффективности.
...
**Шаги алгоритма:**
 ...
 - Валидация (ValidationImpl): 
    - процесс 0 проверяет корректность source и recipient
    - результат валидации рассылается всем процессам через MPI_Bcast
    - все процессы возвращают единый результат

 - Предобработка (PreProcessingImpl): инициализация выходного вектора (очистка)

 - Основное вычисление (RunImpl):
    - Broadcast параметров от root процесса
...
| omp         | 4     | 3.3540  | 0.65    | 16.3%      |
| omp         | 8     | 5.7480  | 0.38    | 4.75%      |

## 8. Вывод
В ходе выполнения работы была создана и протестирована программная реализация задачи передачи данных по кольцевой сетевой топологии. Разработка включала два подхода: последовательную версию (SEQ) с использованием виртуальных топологий MPI (MPI_Cart_create) и параллельную версию (MPI) с явной маршрутизацией и ручным управлением соседями в кольце.

Разработанные алгоритмы корректно решают поставленную задачу, что подтверждается успешным прохождением всех 12 функциональных тестов, включая граничные случаи (пустые данные, передача самому себе, большие объёмы данных, отрицательные числа) и проверкой идентичности результатов между SEQ и MPI версиями. Из полученных результатов видно, что MPI-версия работает медленнее Seq-версии, это связано с высокими накладными расходами на межпроцессное взаимодействие и в Seq-версии используется функция MPI_Cart_create, что так же ускоряет работу.
